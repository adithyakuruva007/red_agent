...
> SMS is **FOSS-only** and **Android-only**. The Play Store variant of Red does not declare `READ_SMS` or `SEND_SMS` and the feature is invisible there — no settings, no tools, no code path. Play Store's SMS/Call Log Permissions policy restricts both permissions to default SMS handlers, which Red is not.

Red on the FOSS Android build can **read** incoming SMS messages and **draft** outgoing SMS for the user to review and send. Read and Send are independent user opt-ins: the user can let Red read SMS without also letting it send, and vice versa. Sending is always gated by an explicit user tap in a review banner — the AI never sends directly.
...
4. When the AI calls one of those tools, a [SmsDraft](../../composeApp/src/commonMain/kotlin/com/inspiredandroid/red/data/SmsModels.kt) is written to the persistent [SmsDraftStore](../../composeApp/src/commonMain/kotlin/com/inspiredandroid/red/data/SmsDraftStore.kt) — **no SMS is sent**. The tool result tells the AI the draft is waiting for user review.
5. A [PendingSmsBanner](../../composeApp/src/commonMain/kotlin/com/inspiredandroid/red/ui/chat/composables/PendingSmsBanner.kt) appears at the top of the chat showing the draft's recipient and body, with **Send** and **Discard** buttons.
