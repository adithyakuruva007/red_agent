...
Users can backup and restore all Red settings via a human-readable JSON file. The feature is available under **Settings > General** at the bottom of the page.
...
- Confirming the dialog opens a native file-save dialog and writes `red-settings.json` containing only the selected sections (plus a `"version": 1` field for forward-compatibility).
