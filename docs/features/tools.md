...
Red's tools feature allows the AI to execute external functions during conversations — web search, notifications, calendar events, shell commands, memory operations, and more. Tools are defined with a schema, executed with safety guards, and managed through per-tool toggles in settings.
...
| `search_files` | Search for files inside a directory (e.g., `/sdcard` on Android or user home on desktop) matching a wildcard pattern | Enabled |
| `read_file` | Read the text content of a file on local storage | Enabled |
| `write_file` | Write or edit the text content of a file on local storage | Enabled |
...
- **Clicking a red-ui action button**: the clicked button shows an inline circular spinner in place of its label until the response arrives; other buttons become disabled.
