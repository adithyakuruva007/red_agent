...
- **File location:** App-private `red_secure_prefs` SharedPreferences
...
- **Service identifier:** `com.inspiredandroid.red`

### Desktop (macOS, Windows, Linux)
- **Mechanism:** `EncryptedFileSettings` — custom file-backed `Settings` implementation
- **Encryption:** AES-256-GCM via `javax.crypto`
- **Key management:** 256-bit random key generated via `SecureRandom`, stored at `~/.red/settings.key`
...
- **File location:** `~/.red/settings.aes`
