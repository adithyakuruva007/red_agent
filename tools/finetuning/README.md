# Mistral Fine-Tuning for Red Dynamic UI

Fine-tune a Mistral model that excels at generating Red's `red-ui` JSON DSL — the 29-component system for interactive dynamic UI screens.

## Overview

Red's dynamic UI lets the AI generate interactive screens (forms, dashboards, quizzes, etc.) using a custom JSON format wrapped in ` ```red-ui ` code fences. Different LLM providers vary in how well they handle this format. Fine-tuning teaches a Mistral model the exact component schema, action system, and layout rules.
...
- A `## Assistant` section with the ideal response containing valid red-ui JSON
...
```bash
RED_INTEGRATION=1 RED_OPENAI_KEY=sk-... RED_ANTHROPIC_KEY=sk-ant-... \
  ./gradlew :composeApp:desktopTest --tests "*RedUiValidationTest.validate*" --info
```
...
```bash
RED_INTEGRATION=1 \
  RED_MISTRAL_KEY=your-key \
  RED_MISTRAL_FT_KEY=your-key \
  RED_MISTRAL_FT_MODEL=ft:open-mistral-7b:your-id:date:hash \
  ./gradlew :composeApp:desktopTest --tests "*RedUiValidationTest.validate*" --info
```

Compare the success rates in the generated report at `build/reports/redui-integration/report.md`.

### Step 4: Use in Red

The fine-tuned model automatically appears in Red's model dropdown:

1. Open Red Settings > Services
2. Add or expand the Mistral service
3. Enter your API key
4. Select the fine-tuned model from the dropdown (it will have an `ft:` prefix)

No code changes needed — Red fetches available models from the Mistral API.
...
```json
{"messages":[{"role":"system","content":"...red-ui system prompt..."},{"role":"user","content":"Show me a form..."},{"role":"assistant","content":"Here's the form:\n\n```red-ui\n{\"type\":\"column\",...}\n```"}]}
```

The system prompt matches what Red sends to the AI at runtime (the red-ui component definitions, action types, and layout rules).
