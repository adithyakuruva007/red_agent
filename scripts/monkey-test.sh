...
ACTIVITY="com.inspiredandroid.red.MainActivity"
EVENT_COUNT="${1:-10000}"
THROTTLE="${2:-50}"

echo "=== Red Monkey Test ==="
...
adb shell monkey \
    ...
    -v -v \
    "$EVENT_COUNT" 2>&1 | tee /tmp/red-monkey-results.txt
...
echo ""
echo "=== Results ==="
CRASHES=$(grep -c "CRASH" /tmp/red-monkey-results.txt 2>/dev/null || echo "0")
ANRS=$(grep -c "ANR" /tmp/red-monkey-results.txt 2>/dev/null || echo "0")
echo "Crashes: $CRASHES"
echo "ANRs:    $ANRS"
echo ""
echo "Full log: /tmp/red-monkey-results.txt"

if [ "$CRASHES" -gt 0 ] || [ "$ANRS" -gt 0 ]; then
    echo ""
    echo "=== Crash Details ==="
    grep -A 10 "CRASH\|ANR" /tmp/red-monkey-results.txt || true
    exit 1
fi
