...
import com.inspiredandroid.red.data.EncryptedFileSettings
...
actual fun getAppFilesDirectory(): String {
    ...
    val redDir = File("$userHome/.red")
    if (!redDir.exists()) {
        redDir.mkdirs()
    }
    return redDir.absolutePath
}
    ...
    try {
        when (currentPlatform as Platform.Desktop) {
            ...
            Platform.Desktop.Windows -> {
                ...
                val trayIcon = java.awt.TrayIcon(image, "Red")
...
val searchFilesTool = object : Tool {
    override val schema = ToolSchema(
        name = "search_files",
        description = "Search for files inside a directory (e.g. user home or work folder) matching a wildcard pattern.",
        parameters = mapOf(
            "directory" to ParameterSchema("string", "The directory path to search in.", true),
