package com.inspiredandroid.red.finetuning
 ...
 * 2. **Saved integration test responses** in `build/reports/redui-integration/` — curated
 *    successful responses from strong LLM providers (run RedUiValidationTest first)
 */
class GenerateTrainingData {
    ...
    fun `generate training data`() {
        ...
        val reportDir = File(projectRoot, "build/reports/redui-integration")
        ...
        if (reportDir.isDirectory) {
            ...
            println("    Run RedUiValidationTest first to collect LLM responses for curation.")
        }
        ...
        for (example in examples) {
            ...
            if (validateRedUi(assistantContent)) {
                valid.add(example)
            } else {
                invalidCount++
                val userMsg = example.messages.firstOrNull { it.role == "user" }?.content?.take(80) ?: "?"
                println("  [SKIP] Invalid red-ui in example: $userMsg")
            }
        }
    ...
    /**
     * Loads successful LLM responses from RedUiValidationTest report directory.
    ...
    private fun loadCuratedResponses(reportDir: File): List<TrainingExample> {
        ...
        for (file in rawFiles) {
            ...
            val hasUi = blocks.any { it is RedUiBlock }
            val hasError = blocks.any { it is RedUiError }
    ...
    /**
     * Validates that an assistant response contains at least one valid red-ui block
     * with no parse errors.
     */
    private fun validateRedUi(response: String): Boolean {
        val blocks = parseMarkdown(response).blocks
        val hasUi = blocks.any { it is RedUiBlock }
        val hasError = blocks.any { it is RedUiError }
        // Allow responses that don't contain red-ui at all (mixed markdown-only for dynamic mode)
    ...
    // region System prompt (mirrors RedUiValidationTest) --------------------------------------

    private enum class Mode { DYNAMIC_UI, INTERACTIVE }

    /**
     * Builds the red-ui system prompt. Mirrors [RedUiValidationTest.buildSystemPrompt].
     */
