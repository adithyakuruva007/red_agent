package com.inspiredandroid.red.integration
...
import com.inspiredandroid.red.network.dtos.anthropic.AnthropicChatRequestDto
import com.inspiredandroid.red.network.dtos.anthropic.extractText
import com.inspiredandroid.red.network.dtos.gemini.GeminiChatRequestDto
import com.inspiredandroid.red.network.dtos.gemini.extractText
import com.inspiredandroid.red.network.dtos.openaicompatible.OpenAICompatibleChatRequestDto
 ...
 * validates that their responses contain well-formed red-ui blocks.
 *
 * By default this test is **skipped** — it only runs when `RED_INTEGRATION=1` is set
 ...
 * RED_INTEGRATION=1 \
 *   RED_OPENAI_KEY=sk-... \
 *   RED_ANTHROPIC_KEY=sk-ant-... \
 *   ./gradlew :composeApp:desktopTest --tests "*RedUiValidationTest*" --info
 * ```
 *
 ...
 * |--------------|-----------------------|--------------------------|------------------------------|
 * | OpenAI       | RED_OPENAI_KEY        | RED_OPENAI_MODEL         | gpt-4o-mini                  |
 * | Anthropic    | RED_ANTHROPIC_KEY     | RED_ANTHROPIC_MODEL      | claude-3-5-haiku-latest      |
 * | Gemini       | RED_GEMINI_KEY        | RED_GEMINI_MODEL         | gemini-2.0-flash             |
 * | Mistral      | RED_MISTRAL_KEY       | RED_MISTRAL_MODEL        | mistral-small-latest         |
 * | Groq         | RED_GROQ_KEY          | RED_GROQ_MODEL           | llama-3.3-70b-versatile      |
 * | OpenRouter   | RED_OPENROUTER_KEY    | RED_OPENROUTER_MODEL     | openai/gpt-4o-mini           |
 * | DeepSeek     | RED_DEEPSEEK_KEY      | RED_DEEPSEEK_MODEL       | deepseek-chat                |
 * | xAI          | RED_XAI_KEY           | RED_XAI_MODEL            | grok-2-latest                |
 * | Cerebras     | RED_CEREBRAS_KEY      | RED_CEREBRAS_MODEL       | llama-3.3-70b                |
 * | Moonshot     | RED_MOONSHOT_KEY      | RED_MOONSHOT_MODEL       | moonshot-v1-8k               |
 * | Together     | RED_TOGETHER_KEY      | RED_TOGETHER_MODEL       | meta-llama/Llama-3.3-70B-Instruct-Turbo |
 * | Mistral FT   | RED_MISTRAL_FT_KEY    | RED_MISTRAL_FT_MODEL     | ft:open-mistral-7b:latest  |
 *
 * Additional knobs:
 * - `RED_REPORT_DIR` — where to write the report (default: `build/reports/redui-integration`)
 * - `RED_MIN_SUCCESS_RATE` — 0.0-1.0, fail the test below this rate (default: 0.0, never fail)
 * - `RED_REPLAY_STRICT` — set to `1` to make the replay test fail when any saved response
 ...
 * 4. When green, copy the now-passing payloads into `RedUiParserTest` as regressions
 */
class RedUiValidationTest {

    @Test
    fun `validate red-ui output across providers`() {
        if (System.getenv("RED_INTEGRATION") != "1") {
            println("[RedUiValidationTest] Skipped — set RED_INTEGRATION=1 to run.")
            return
        }

        val providers = discoverProviders()
        if (providers.isEmpty()) {
            println("[RedUiValidationTest] Skipped — no provider API keys found in env.")
            println("    Set any of: RED_OPENAI_KEY, RED_ANTHROPIC_KEY, RED_GEMINI_KEY, RED_GROQ_KEY, RED_OPENROUTER_KEY")
            return
        }

        val reportDir = File(System.getenv("RED_REPORT_DIR") ?: "build/reports/redui-integration")
        reportDir.mkdirs()
        val requests = Requests()
        val allResults = mutableListOf<PromptResult>()

        for (provider in providers) {
            println("\n[RedUiValidationTest] === ${provider.label} (${provider.modelId}) ===")
        ...
        val minRate = System.getenv("RED_MIN_SUCCESS_RATE")?.toDoubleOrNull() ?: 0.0
        if (minRate > 0.0) {
     ...
     *
     * Skips silently if no saved responses are found. Set `RED_REPLAY_STRICT=1` to make
    ...
    fun `replay saved responses`() {
        ...
        if (!reportDir.isDirectory) {
            println("[RedUiValidationTest] Replay skipped — no report directory at ${reportDir.absolutePath}")
            return
        }
        ...
        if (rawFiles.isEmpty()) {
            println("[RedUiValidationTest] Replay skipped — no .raw.txt files under ${reportDir.absolutePath}")
            println("    Run the online test first with RED_INTEGRATION=1 and at least one provider key.")
            return
        }

        println("\n[RedUiValidationTest] Replaying ${rawFiles.size} saved responses from ${reportDir.absolutePath}")
        var success = 0
        var partial = 0
        var noUi = 0
        var parseError = 0
        ...
        for (file in rawFiles) {
            ...
            val ui = blocks.filterIsInstance<RedUiBlock>().size
            val err = blocks.filterIsInstance<RedUiError>().size
        ...
        println("\n[RedUiValidationTest] Replay summary: $success ok, $partial partial, $noUi no-ui, $parseError parse-error  (total ${rawFiles.size})")

        val strict = System.getenv("RED_REPLAY_STRICT") == "1"
    ...
    private fun discoverProviders(): List<ProviderSpec> {
        ...
        env("RED_OPENAI_KEY")?.let {
            out += ProviderSpec(
                "openai",
                "OpenAI",
                Service.OpenAI,
                env("RED_OPENAI_MODEL") ?: "gpt-4o-mini",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("RED_ANTHROPIC_KEY")?.let {
            out += ProviderSpec(
                "anthropic",
                "Anthropic",
                Service.Anthropic,
                env("RED_ANTHROPIC_MODEL") ?: "claude-3-5-haiku-latest",
                it,
                ProviderSpec.Kind.ANTHROPIC,
            )
        }
        env("RED_GEMINI_KEY")?.let {
            out += ProviderSpec(
                "gemini",
                "Gemini",
                Service.Gemini,
                env("RED_GEMINI_MODEL") ?: "gemini-2.0-flash",
                it,
                ProviderSpec.Kind.GEMINI,
            )
        }
        env("RED_MISTRAL_KEY")?.let {
            out += ProviderSpec(
                "mistral",
                "Mistral",
                Service.Mistral,
                env("RED_MISTRAL_MODEL") ?: "mistral-small-latest",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("RED_GROQ_KEY")?.let {
            out += ProviderSpec(
                "groq",
                "Groq",
                Service.Groq,
                env("RED_GROQ_MODEL") ?: "llama-3.3-70b-versatile",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("RED_OPENROUTER_KEY")?.let {
            out += ProviderSpec(
                "openrouter",
                "OpenRouter",
                Service.OpenRouter,
                env("RED_OPENROUTER_MODEL") ?: "openai/gpt-4o-mini",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("RED_DEEPSEEK_KEY")?.let {
            out += ProviderSpec(
                "deepseek",
                "DeepSeek",
                Service.DeepSeek,
                env("RED_DEEPSEEK_MODEL") ?: "deepseek-chat",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("RED_XAI_KEY")?.let {
            out += ProviderSpec(
                ...
                "xAI",
                Service.XAI,
                env("RED_XAI_MODEL") ?: "grok-2-latest",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("RED_CEREBRAS_KEY")?.let {
            out += ProviderSpec(
                "cerebras",
                "Cerebras",
                Service.Cerebras,
                env("RED_CEREBRAS_MODEL") ?: "llama-3.3-70b",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("RED_MOONSHOT_KEY")?.let {
            out += ProviderSpec(
                "moonshot",
                "Moonshot",
                Service.Moonshot,
                env("RED_MOONSHOT_MODEL") ?: "moonshot-v1-8k",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("RED_TOGETHER_KEY")?.let {
            out += ProviderSpec(
                "together",
                "Together",
                Service.Together,
                env("RED_TOGETHER_MODEL") ?: "meta-llama/Llama-3.3-70B-Instruct-Turbo",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("RED_MISTRAL_FT_KEY")?.let {
            out += ProviderSpec(
                "mistral-ft",
                "Mistral Fine-tuned",
                Service.Mistral,
                env("RED_MISTRAL_FT_MODEL") ?: "ft:open-mistral-7b:latest",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
    ...
    private fun runPrompt(
        ...
        val blocks = parseMarkdown(raw).blocks
        val uiBlocks = blocks.filterIsInstance<RedUiBlock>()
        val errorBlocks = blocks.filterIsInstance<RedUiError>()
    ...
    private fun writeReport(reportDir: File, results: List<PromptResult>, providers: List<ProviderSpec>) {
        val report = buildString {
            appendLine("# red-ui Integration Report")
            appendLine()
        ...
        println("\n[RedUiValidationTest] Report written to: ${File(reportDir, "report.md").absolutePath}")
    }

    private fun printSummary(results: List<PromptResult>, providers: List<ProviderSpec>) {
        println("\n[RedUiValidationTest] === SUMMARY ===")
        for (p in providers) {
            val r = results.filter { it.provider.slug == p.slug }
    ...
    /**
     * Duplicates the red-ui portion of the app's system prompt. Mirrors
    ...
    private fun buildSystemPrompt(mode: TestPrompt.Mode): String = buildString {
        ...
        if (dynamicUiOnly) {
            append("\n## Dynamic UI\n")
            append("You can enhance your chat responses with interactive UI elements using red-ui blocks. ")
            ...
            append("Use red-ui whenever collecting data, offering choices, presenting structured information, or guiding multi-step workflows. ")
            append("You can mix red-ui blocks with regular markdown text naturally — use markdown for explanations and red-ui for interactive elements.\n\n")
        } else {
            append("\n## Interactive UI Mode (ACTIVE)\n")
            append("You are in full-screen interactive UI mode. The user ONLY sees rendered red-ui components — they cannot see markdown, plain text, or anything outside a red-ui fence.\n")
            append("Your ENTIRE response must be a single ```red-ui code fence. No text before it, no text after it, no markdown. If you write anything outside the fence, the user will NOT see it.\n\n")
        }

        append("Format: wrap a JSON object in ```red-ui fences.\n\n")
        ...
        if (dynamicUiOnly) {
            ...
            append("Rules:\n")
            append("- Each response is a COMPLETE screen layout. Include all content and actions in one red-ui block.\n")
    ...
    companion object {
        private val TEST_PROMPTS = listOf(
            TestPrompt(
                "simple-question",
                "Simple question expecting a text response with red-ui elements",
            ...
            TestPrompt(
                "mixed-markdown",
                "Response that mixes markdown explanation with red-ui interactive block",
