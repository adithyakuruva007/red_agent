package com.inspiredandroid.red.sandbox
...
class SshConfigManagerTest {
    ...
    fun ensureDefaultsCreatesBlockOnce() {
        ...
        assertContains(first, "# red:defaults:start")
        ...
        assertContains(first, "# red:defaults:end")
    ...
    fun upsertHostAddsBlockAndSeedsDefaults() {
        ...
        assertTrue(changed)
        val text = configText()
        assertContains(text, "# red:defaults:start")
        assertContains(text, "# red:host:prod:start")
        assertContains(text, "Host prod")
        assertContains(text, "HostName 1.2.3.4")
        assertContains(text, "# red:host:prod:end")
    }
    ...
    fun upsertHostReplacesPreviousBlock() {
        ...
        val startCount = "# red:host:prod:start".toRegex().findAll(text).count()
        val endCount = "# red:host:prod:end".toRegex().findAll(text).count()
    ...
    @Test
    fun upsertHostKeepsNonRedContent() {
