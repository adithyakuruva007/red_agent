 ...
 * [com.inspiredandroid.red.tools.SshConfigureHostTool].
 *
 ...
 *
 * All blocks we own are bracketed with `# red:<marker>:start` /
 * `# red:<marker>:end` so upserts are idempotent without disturbing
...
class SshConfigManager(private val homeDir: File) {
    ...
    fun listAliases(): List<String> {
        val regex = Regex("""# red:host:([^:\s]+):start""")
    ...
    private fun upsertBlock(text: String, marker: String, body: String): String {
        val startTag = "# red:$marker:start"
        val endTag = "# red:$marker:end"
