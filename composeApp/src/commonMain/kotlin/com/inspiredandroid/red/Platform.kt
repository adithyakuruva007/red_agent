...
expect fun Modifier.platformHover(onEnter: () -> Unit, onExit: () -> Unit): Modifier

interface PlatformCameraLauncher {
    fun launch()
}

@Composable
expect fun rememberPlatformCameraLauncher(onResult: (PlatformFile?) -> Unit): PlatformCameraLauncher
...
/**
 * True only on the FOSS Android build. Gated on `RedNotificationListenerService`
