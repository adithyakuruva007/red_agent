...
fun AppSettings.currentService(): Service {
    ...
    if (id.isEmpty()) {
        return getConfiguredServiceInstances().firstOrNull()?.let { Service.fromId(it.serviceId) } ?: throw IllegalStateException("No configured services available")
    }
...
fun AppSettings.getBaseUrl(service: Service): String = when (service) {
    ...
    Service.OllamaLocal -> settings.getString(service.baseUrlKey, "http://localhost:11434")
    else -> ""
}

fun AppSettings.setBaseUrl(service: Service, baseUrl: String) {
    if (service == Service.OpenAICompatible || service == Service.OllamaLocal) {
