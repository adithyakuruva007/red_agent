package com.inspiredandroid.red.skills
...
class SkillManager(
    ...
    init {
        // Load once the sandbox is installed. On platforms without a sandbox (like Desktop),
        // fallback to loading only built-in skills so slash suggestions still work.
        scope.launch {
            var wasInstalled = false
            sandboxController.status.collect { status ->
                if (status.installed) {
                    if (!wasInstalled) load()
                } else {
                    loadBuiltInsOnly()
                }
    ...
    private suspend fun loadBuiltInsOnly() {
        val builtIns = loadBuiltInSkills()
        _skills.value = builtIns.sortedBy { it.id }
    }
