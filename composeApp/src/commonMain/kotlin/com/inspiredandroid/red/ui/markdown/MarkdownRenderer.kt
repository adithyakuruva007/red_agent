...
import com.inspiredandroid.red.ui.markdown.math.MathFormula
 ...
 *
 * RedUI blocks dispatch to [RedUiRenderer]; pass `isInteractive = false` to render them as
...
fun MarkdownContent(
    ...
    val doc = remember(content) {
        runCatching { parseMarkdown(content) }.getOrElse {
            MarkdownDocument(persistentListOf(Paragraph(persistentListOf(com.inspiredandroid.red.ui.markdown.Text(content)))))
        }
    }
