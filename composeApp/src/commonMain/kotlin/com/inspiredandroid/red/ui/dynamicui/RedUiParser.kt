...
/**
 * Decodes the body of a `red-ui` fenced JSON block into a [RedUiNode].
 *
 * The parse pipeline runs each block through three stages:
 *   1. **Block extraction** — the markdown parser locates `red-ui` fences and hands the raw
 ...
 *      [parseNode] in `RedUiNodeBuilders.kt`, constructing [RedUiNode] instances field-by-field.
...
object RedUiParser {
    ...
    /** Result of decoding a red-ui fence body; consumed by the markdown parser. */
    sealed interface UiBlockResult {
        data class Ui(val node: RedUiNode, val rawJson: String) : UiBlockResult
        data class Error(val rawJson: String) : UiBlockResult
    }

    /**
     * Decode the raw body of a red-ui fence (everything between the opening and closing triple
     * backticks). Returns either a decoded [RedUiNode] or an [UiBlockResult.Error] carrying the
    ...
    fun parseUiBlockBody(rawBlock: String): UiBlockResult? {
        ...
        return try {
            ...
            println("red-ui parse error: ${e.message} | ${json.take(500)}")
    ...
    private fun tryParseLine(line: String): RedUiNode? = runCatching { parseSingleNode(line) }.getOrNull()
        ?: runCatching { parseSingleNode(sanitizeJson(line)) }.getOrNull()
        ?: run {
            println("red-ui parse error: failed to deserialize line | ${line.take(500)}")
            null
        }

    /** Parse a repaired JSON string into a [RedUiNode] via the direct builder pipeline. */
    private fun parseSingleNode(json: String): RedUiNode? = parseNode(SharedJson.parseToJsonElement(json))
