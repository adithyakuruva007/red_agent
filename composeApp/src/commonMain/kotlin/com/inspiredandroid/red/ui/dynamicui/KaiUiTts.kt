...
import com.inspiredandroid.red.ui.markdown.toSpeakableText

/**
 * TTS-friendly text for a message that may contain red-ui fences. Routed through the unified
 * markdown parser: formatting is stripped, code blocks dropped, and red-ui blocks walked for
...
internal fun RedUiNode.collectSpeakableText(): String {
    val parts = mutableListOf<String>()
    walk(parts)
    return parts.asSequence().filter { it.isNotBlank() }.joinToString(". ")
}

private fun RedUiNode.walk(parts: MutableList<String>) {
    when (this) {
