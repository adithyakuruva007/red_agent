...
@objc public class RedLiteRTBridge: NSObject, LiteRTSwiftBridge {
    ...
    private static func parseMessages(_ json: String) throws -> [Message] {
        guard let data = json.data(using: .utf8) else {
            throw NSError(domain: "RedLiteRTBridge", code: 1, userInfo: [NSLocalizedDescriptionKey: "Invalid UTF-8 in messages JSON"])
        }
...
@objc public class RedLiteRTBridgeInstaller: NSObject {
    @objc public static func install() {
        LiteRTBridgeRegistry.shared.bridge = RedLiteRTBridge()
    }
}
